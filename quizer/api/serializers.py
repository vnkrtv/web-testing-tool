...
class TestSerializer(serializers.Serializer):
    ...
    def to_representation(self, test):
        ...
        test_results = TestResult.objects.filter(
            is_running=True, test__id=test.id
        ).first()
        if test_results:
...
class UserResultSerializer(serializers.Serializer):
    ...
    class Meta:
        model = UserResult
        read_only_fields = ("id", "testing_result_id")
...
class TestResultSerializer(serializers.Serializer):
    ...
    def get_results(self, test_results) -> List[Dict[str, Any]]:
        serializer = UserResultSerializer(
            UserResult.objects.filter(testing_result=test_results), many=True
        )
        return serializer.data

    def create(self, validated_data):
        launched_lecturer = User.objects.get(
            id=validated_data.get("launched_lecturer_id")
        )
        subject = Subject.objects.get(id=validated_data.get("subject_id"))
        test = Test.objects.get(id=validated_data.get("test_id"))
    ...
    class Meta:
        model = TestResult
        read_only_fields = ("id", "launched_lecturer", "subject", "results")
